...
class testapi(RunMethod):
    def __init__(self):
        self.base_url='http://10.100.10.198:8556'
        ...
        if name :
            try:
                ...
                if site == "response-data" and response:
                    ...
                    if pattern:
                        self.variate_dict[key] = pattern[0]
                        # 头部信息添加token
                        if key == "Authentication":
                            self.headers[key] = pattern[0]
                        logging.info("添加变量成功："+str(self.variate_dict))
    ...
    def api_assert(self,r, eqs):
        ...
        for e in eq:
            ...
            # 判断response返回码
            if  type(eqs) is float or  e.isdigit() :
    ...
    def one_test(self,case,test_case):
        ...
        logging.info("开始执行 "+case["TEST_ID"] +" 用例")
        # 头部变量信息获取
        self.variate(case["variate"])
        # 判断data中是否需要将变量替换
        case["data"] = self.judge(case["data"])
        logging.info("当前请求数据为 ： "+ str(case["data"]))
        ...
        try:
            ...
            # 断言后excel更新断言状态
            logging.info("当前更新 " + case["TEST_ID"] + " 用例 eq_result")
        ...
        for case in test_case:
            ...
            if case["isshow"] == "T":
                ...
                if case["test_set"]:
                    ...
                    logging.info("当前用例集为 "+ str(test_set))
                    ...
                    # 断言后excel更新断言状态
                    case_index = test_case.index(case)
                    logging.info("当前更新第" + case["TEST_ID"]  + "条用例 eq_result")
